let shoppingItems;
const itemsArray = document.getElementById("shopping-items");
...
fetch('https://fakestoreapi.com/products')
  ...
  .then(itemsJsonData => {
    ...
    itemsArray.innerHTML = shoppingItems.map((item) => {
      if (parseInt(item.rating.rate) > 3.9) {
        return (
          `
          <div>  
  ...
  .then(() => {
    const items = document.querySelectorAll(".item:not(#add-to-basket)");
    items.forEach(item => {
